...
**Testing.** The suite lives in `tests/`; run it with `bun test`. Type-check with `bun run typecheck`. UI assertions render through `createTestRenderer` from `@opentui/core/testing` and check the captured char frame — text content only resolves inside `bun test`, not one-shot `bun run` renders.
