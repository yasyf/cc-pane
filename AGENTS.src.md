...
Single pane of glass for Claude Code and all the cc-* helpers. A full-screen
OpenTUI dashboard, run with bun (`bun start`), that orchestrates Claude Code
session backends (cmux, superset) and renders each session's meta in one view.
...
**Testing.** The suite lives in `tests/`; run it with `bun test`. Type-check with `bun run typecheck`. UI assertions render through `createTestRenderer` from `@opentui/core/testing` and check the captured frame — note text content only resolves inside `bun test`, not one-shot `bun run` renders.
